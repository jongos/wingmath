...
The public repository's `.github/workflows/sync-data.yml` checks `/api/public-data` every five minutes, on a workflow change, and on manual dispatch. It commits a new `data/current/database.json` only when the curated payload changes. Git history preserves previous versions. This is a periodic mirror, not an instantaneous database transaction hook; scheduled jobs can be delayed by GitHub. Changes between checks can be coalesced. A failed download or validation leaves the last good snapshot unchanged and fails the workflow visibly.
